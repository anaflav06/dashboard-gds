...
def gerar_recibo_pdf(
    ...
    elementos.append(Paragraph("Controle diário de ENTREGAS por terceiros", style_title))
    ...
    if usar_base_detalhada:
        for data_rota, df_dia_atual in df.groupby(df["Data Rota"].dt.date, dropna=False):
            df_dia_atual = df_dia_atual.copy()

            # Entregas normais separadas por valor real da tabela de CEP.
            resumo_valor = (
                df_dia_atual.groupby("Valor CEP", dropna=False)
                .agg(
                    Quantidade=("Pedido", "size"),
                    Total_Entregas=("Valor CEP", "sum"),
                )
                .reset_index()
                .sort_values("Valor CEP")
            )

            primeira_linha_dia = True
            for _, item in resumo_valor.iterrows():
                valor_cep = float(item["Valor CEP"] or 0)
                qtd = int(item["Quantidade"] or 0)
                total_linha = float(item["Total_Entregas"] or 0)
                linhas.append([
                    fmt_data(data_rota) if primeira_linha_dia else "",
                    "Entregas realizadas",
                    moeda(valor_cep),
                    str(qtd),
                    moeda(total_linha),
                ])
                primeira_linha_dia = False

            kg_excedente = float(df_dia_atual["KG Excedente"].sum())
            valor_kg = float(df_dia_atual["Valor Excedente KG"].sum())
            if kg_excedente > 0:
            ...
            total_dia = float(df_dia_atual["Total Entrega"].sum())
            linhas_estilo_total_dia.append(len(linhas))
            linhas.append(["", "Total do dia", "", "", moeda(total_dia)])
    else:
        for _, row in df.iterrows():
            qtd = int(row.get("Quantidade_Entregas", 0) or 0)
            valor_entregas = float(row.get("Valor_Entregas", 0) or 0)
            kg_excedente = float(row.get("KG_Excedente_Calculado", 0) or 0)
            valor_kg = float(row.get("Valor_KG_Excedente", 0) or 0)
            total_dia = float(row.get("Total_Dia", 0) or 0)

            linhas.append([
                fmt_data(row.get("Data Rota")),
                "Entregas realizadas",
                "Conforme CEP",
                str(qtd),
                moeda(valor_entregas),
            ])
...
motorista_filtro = st.multiselect("Filtrar entregador", motoristas, default=motoristas)
...
st.markdown("---")
st.markdown('<div class="section-heading">Gerar recibo</div>', unsafe_allow_html=True)
...
if df_recibo.empty:
    ...
    st.download_button(
        "🧾 Gerar recibo em PDF",
        data=recibo_pdf,
        file_name=f"recibo_{nome_motorista_arquivo}_{normalizar_nome_coluna(quinzena_recibo)}_{data_inicio_recibo.strftime('%Y%m%d')}_{data_fim_recibo.strftime('%Y%m%d')}.pdf",
...
cols_show = [
    ...
    "CEP", "Placa", "Tipo Veículo Final", "Tipo Veículo Assumido",
